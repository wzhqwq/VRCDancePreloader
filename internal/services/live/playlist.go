...
func (s *Server) handlePlaylist(w http.ResponseWriter, _ *http.Request) {
	pl := playlist.GetCurrentPlaylist()
	writeOk(w, lo.Map(pl.GetItemsSnapshot(), func(item *song.StatefulSong, _ int) song.LiveFullInfo {
...
func NewPlaylistWatcher(s *WsService, pl *playlist.PlayList) *PlaylistWatcher {
	w := &PlaylistWatcher{
		s:  s,
		pl: pl,
...
func (w *PlaylistWatcher) UpdateItems() {
	...
	songs := lo.Map(items, func(item *song.StatefulSong, index int) *SongWatcher {
