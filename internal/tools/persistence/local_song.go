...
func (f *LocalSongs) LoadFavorites() error {
	f.Lock()
	defer f.Unlock()
...
//
//	// load from db
//	rows, err := localSongTable.Query(listFavoriteIdsInPyPy, internal_id.PyPyInternalPrefix + "%")
//	if err != nil {
//		logger.ErrorLn("Failed to load favorite entries:", err)
//		return ""
//	}
...
func InitLocalSongs() {
	...
	err := currentLocalSongs.LoadFavorites()
	if err != nil {
		logger.ErrorLn("Failed to load favorite entries:", err)
	}
}
