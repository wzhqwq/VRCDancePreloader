...
func duDuPostProcess() {
	...
	if lastQueue != "" && lastUserData != "" {
		if queueChanged {
			...
			diffQueues(playlist2.GetQueue(), newQueue)
		}
	}
...
func syncDuDuVideoUsingCountdown(pair string) {
	...
	if playlist2.MarkURLPlaying(userDataDuDu.URL, -countDown) {
		if countDown > 0 {
...
func justPlayDuDuVideo() {
	if playlist2.MarkURLPlayingWithoutSync(userDataDuDu.URL) {
