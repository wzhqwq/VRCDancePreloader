...
func diffQueues(old []*song.StatefulSong, new []queue.Item) {
	if len(old) == len(new) {
		allTheSame := true
		for i := 0; i < len(old); i++ {
	...
	logger.InfoLn(
		"playlist changed:",
		lo.Map(old, func(item *song.StatefulSong, _ int) string {
			return item.SongId()
		}),
		"->",
		lo.Map(new, func(item queue.Item, _ int) string {
	...
	var deletions []int
	reused := make([]*song.StatefulSong, len(new))
	...
	newList := lo.Map(reused, func(item *song.StatefulSong, index int) *song.StatefulSong {
		if item != nil {
			return item
		}
		// create if empty
	...
	if len(old) > 0 && (len(new) == 0 || !new[0].MatchWithPreloaded(old[0])) {
		old[0].StateMachine().CancelPlayingLoop()
	}
}
