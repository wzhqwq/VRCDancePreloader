...
type Check struct {
	...
	setting interactive.StatefulSetting[bool]
...
func NewCheck(label string, setting interactive.StatefulSetting[bool]) *Check {
	c := &Check{
		Label:   label,
		Checked: setting.Get(),
...
func (c *Check) Hide() {
	if c.focused {
		...
		if c := fyne.CurrentApp().Driver().CanvasForObject(c); c != nil {
			c.Focus(nil)
		}
	}
...
func (c *Check) Tapped(pe *fyne.PointEvent) {
	...
	if !c.focused {
		if can := fyne.CurrentApp().Driver().CanvasForObject(c); can != nil {
			can.Focus(c)
		}
	}
...
func (c *Check) CreateRenderer() fyne.WidgetRenderer {
	...
	text := canvas.NewText(c.Label, th.Color(theme.ColorNameForeground, v))
	text.Alignment = fyne.TextAlignLeading

	focusIndicator := canvas.NewCircle(th.Color(theme.ColorNameBackground, v))
	r := &checkRenderer{
		objects:        []fyne.CanvasObject{focusIndicator, bg, icon, text},
		bg:             bg,
		icon:           icon,
		label:          text,
...
func (c *Check) updateValue(checked bool) {
