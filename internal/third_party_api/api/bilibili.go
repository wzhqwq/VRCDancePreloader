...
func GetPlayerInfo(bvID string, ctx context.Context) (*BiliPlayerInfo, error) {
	info, err := GetBvInfo(bvID, ctx)
	if err != nil {
		return nil, err
	}
	return requestBiliApi[BiliPlayerInfo](utils.GetBiliVideoPlayerURL(bvID, info.Cid), bvID, ctx)
}
