...
func GetYoutubeInfoFromApi(videoID, apiKey string) (*youtube.Video, error) {
	...
	ctx := requesting.GetYoutubeApiContext(timeoutCtx)

	svc, err := youtube.NewService(ctx, requesting.WithYoutubeApiClient(apiKey))
	if err != nil {
		return nil, err
	}
...
func GetYoutubeTitleFromApi(videoID, apiKey string) (string, error) {
	info, err := GetYoutubeInfoFromApi(videoID, apiKey)
	if err != nil {
		return "", err
	}
...
func GetYoutubeDurationFromApi(videoID, apiKey string) (time.Duration, error) {
	info, err := GetYoutubeInfoFromApi(videoID, apiKey)
	if err != nil {
		return 0, err
	}
